...
function getValidationFieldRules(validationSchemaFile, definitionName, fieldName) {
    if (!validationSchemaFile || !validationSchemaFile[definitionName] || !validationSchemaFile[definitionName][fieldName]) return null
...
function graphQLSchemaToValidationSchema(validationSchemaFile, graphqlSchema) {
    const schema = {}

    for (const [definitionName, definition] of Object.entries(graphqlSchema)) {
        
        var fields = {}
        for (const [fieldName, fieldBody] of Object.entries(definition.fields)) {
            var validationFieldRules = getValidationFieldRules(validationSchemaFile, definitionName, fieldName)
...
export default graphQLSchemaToValidationSchema;